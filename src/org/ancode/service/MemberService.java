...
import org.ancode.util.Method;
import org.ancode.util.ThreadUtil;
import org.ancode.util.AbstractNetImage;
import org.ancode.util.JpegNetImage;
import org.ancode.util.Log;
import org.ancode.util.DateUtils;
import org.ancode.util.QQEnvironment;
import org.ancode.util.ImageUtil;
...
public class MemberService {
    ...
    public ImageIcon downloadFace(Member member) throws Exception {
        ...
        if (srcFile.exists() && !destFile.exists()) {
            InputStream in = this.getClass().getResourceAsStream("/res/images/status/" + statusWatermark);
    ...
    public ImageIcon loadDefFace(Member member) throws Exception {
        ...
        if (f.exists()) {
            ...
            try {
                URL filename = this.getClass().getClassLoader().getResource("／res/images/status/" + statusWatermark.substring(0, statusWatermark.indexOf(".")) + ".jpg").toURI().toURL();
