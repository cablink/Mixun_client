...
public class NettyClient extends Thread implements OnSharedPreferenceChangeListener {
	// 用来查看是否接受到重复信息
	private URI uri;
	...
	public long getTimeOffset2Server() {
		...
		try {
			...
			String result = SXmlParser.xmlResponseTag(
					SCore.getInstance().getRequest("/gettime/" + SConfig.getInstance().getSessionKey() + "/call.xml"),
					"time");
			if (result != null) {
